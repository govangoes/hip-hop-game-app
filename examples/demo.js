...
const PackOpeningService_1 = require("../src/services/PackOpeningService");
const CollectionService_1 = require("../src/services/CollectionService");
const TradingService_1 = require("../src/services/TradingService");
...
async function runDemo() {
    ...
    // Initialize services
    const packService = new PackOpeningService_1.PackOpeningService();
    const collectionService = new CollectionService_1.CollectionService();
    const tradingService = new TradingService_1.TradingService();
    ...
    console.log('-'.repeat(60));
    const standardPack = sampleData_1.samplePacks[0];
    ...
    const packResult1 = await packService.openPack(standardPack, user1Collection.userId, sampleData_1.sampleItems, user1Collection);
    console.log('\n✨ Items Received:');
    ...
    console.log('-'.repeat(60));
    const premiumPack = sampleData_1.samplePacks[1];
    ...
    for (let i = 0; i < 3; i++) {
        const packResult = await packService.openPack(premiumPack, user1Collection.userId, sampleData_1.sampleItems, user1Collection);
    ...
    console.log('-'.repeat(60));
    const collectionValue = collectionService.calculateCollectionValue(user1Collection, sampleData_1.sampleItems);
    ...
    user1Collection.items.forEach(userItem => {
        const item = sampleData_1.sampleItems.find(i => i.id === userItem.collectibleId);
        if (item) {
    ...
    if (duplicates.length > 0) {
        ...
        duplicates.forEach(dup => {
            const item = sampleData_1.sampleItems.find(i => i.id === dup.collectibleId);
            if (item) {
    ...
    sampleData_1.sampleAlbums.forEach(album => {
        ...
        if (isComplete) {
            ...
            progress.missingItems.forEach(itemId => {
                const item = sampleData_1.sampleItems.find(i => i.id === itemId);
                if (item) {
                    console.log(`       - ${item.name} (${item.rarity})`);
                }
            });
    ...
    for (let i = 0; i < 2; i++) {
        const packResult = await packService.openPack(standardPack, user2Collection.userId, sampleData_1.sampleItems, user2Collection);
        packResult.itemsReceived.forEach(item => {
    ...
    if (user1Duplicates.length > 0 && user2Items.length > 0) {
        ...
        const offeredItemDetails = sampleData_1.sampleItems.find(i => i.id === offeredItem.collectibleId);
        const requestedItemDetails = sampleData_1.sampleItems.find(i => i.id === requestedItem.collectibleId);
        console.log('\n💱 Trade Proposal:');
        console.log(`  User 1 offers: ${offeredItemDetails?.name}`);
        console.log(`  User 1 requests: ${requestedItemDetails?.name}`);
        try {
            const trade = await tradingService.createTradeOffer(user1Collection.userId, user2Collection.userId, [offeredItem.collectibleId], [requestedItem.collectibleId], user1Collection, user2Collection);
    ...
    const newEvent = await eventService.createEvent('Hip-Hop Hall of Fame Week', 'Exclusive legendary artists available for one week only!', 'LIMITED_TIME', new Date(), new Date(Date.now() + 7 * 24 * 60 * 60 * 1000), // 7 days from now
    ['item-001', 'item-002', 'item-003'], // Exclusive legendary artists
    ['pack-special-legends'], ['album-hall-of-fame']);
    console.log(`\n  🎉 Event Created!`);
    ...
    console.log(`\n📈 Statistics:`);
    console.log(`  Total Items in System: ${sampleData_1.sampleItems.length}`);
    console.log(`  Available Packs: ${sampleData_1.samplePacks.length}`);
    console.log(`  Completable Albums: ${sampleData_1.sampleAlbums.length}`);
